...
--[[1773]] L["    网易有爱（163UI）是网易游戏频道隆重推出的新一代整合插件。其设计理念是兼顾整合插件的易用性和单体插件的灵活性，同时适合普通和高级用户群体。|n|n    功能上，网易有爱实现了任意插件的随需加载，并可先进入游戏再逐一加载插件，此为全球首创。此外还有标签分类、拼音检索、界面缩排等特色功能。"] = "    網易有愛（163UI）是網易遊戲頻道隆重推出的新一代整合插件。其設計理念是兼顧整合插件的易用性和單體插件的靈活性，同時適合普通和高級用戶群體。|n|n    功能上，網易有愛實現了任意插件的隨需載入，並可先進入遊戲再逐一載入插件，此為全球首創。此外還有標籤分類、拼音檢索、介面縮排等特色功能。"
