...
function DataMake(allowCrossRealm)
  ...
  local function Name(name)
    ...
    --name = decode(name)
    --if not Profile.LocomotiveData then
    --  Profile.LocomotiveData = {}
    --end
    --if not Profile.LocomotiveData[currentRealm] then
    --    Profile.LocomotiveData[currentRealm] = {}
    --end
    --Profile.LocomotiveData[currentRealm][name] = { level = currentLevel, roomID = currentRoomID, bgID = currentBgID }
  end
...
function IsKeyCombinationAndSingleKeyUnbound(keyCombination)
    if IsSingleKeyBound(keyCombination) then
        return false
    end

    local singleKey = keyCombination:match("[^%-]+$")
    if singleKey then
        if IsSingleKeyBound(singleKey) then
            return false
        end
    end
...
function FindFirstTwoUnboundKeys()
    ...
    for _, key in ipairs(keyCombinations) do
        if IsKeyCombinationAndSingleKeyUnbound(key) then
...
function GetGuildMasterRealm()
    for i = 1, GetNumGuildMembers() do
        ...
        if rankIndex == 0 then  -- 0 表示会长
            ...
            return realm or GetRealmName()  -- 如果无服务器名则返回当前服务器
        end
    end
    return nil  -- 如果没有找到会长
end
...
function GetActivityCategoryName(activity)
  ...
  if categoryID == 2 then -- 地下城
    categoryName = "地下城"
  elseif categoryID == 3 then -- 团队副本
    categoryName = "团队副本"
  elseif categoryID == 121 then  -- 地下堡
    categoryName = "地下堡"
  elseif categoryID == 1 then -- 任务
    categoryName = "任务"
  end
