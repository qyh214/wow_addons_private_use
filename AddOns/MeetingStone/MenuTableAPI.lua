...
function ListOfDungeons(menuType)
  ...
  for k, groupId in ipairs(OTHER_DUNGEONS) do
    if OTHER_ACTIVITYS[k] then
      local data = {}
      local actInfo = C_LFGList.GetActivityInfoTable(OTHER_ACTIVITYS[k])
      if actInfo then
        data.text = actInfo.fullName -- C_LFGList.GetActivityGroupInfo(groupId)
        data.fullName = actInfo.fullName -- data.text
        data.categoryId = 2
        data.groupId = groupId
        data.activityId = OTHER_ACTIVITYS[k]
        data.baseFilter = 4
        data.customId = 0
        data.notClickable = true
        data.value = format('2-%d-%d-0', groupId, OTHER_ACTIVITYS[k])
        if data then
          local item = {
            categoryId = data.categoryId,
            groupId = groupId,
