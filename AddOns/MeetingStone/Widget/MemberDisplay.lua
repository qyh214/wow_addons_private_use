...
function MemberDisplay:Constructor()
    local DataDisplay = CreateFrame('Frame', nil, self, 'LFGListGroupDataDisplayTemplate') do
        ...
        -- 创建逃亡者标记图标
        local leaverFrame = CreateFrame("Frame", nil, DataDisplay)
        leaverFrame:SetSize(18, 16)
        leaverFrame:SetPoint("LEFT", DataDisplay, "RIGHT", -5, 0)

        local bg = leaverFrame:CreateTexture(nil, "BACKGROUND")
        bg:SetAllPoints()

        local icon = leaverFrame:CreateTexture(nil, "ARTWORK")
        icon:SetAllPoints()
        icon:SetTexture("Interface/AddOns/MeetingStone/Media/LFG-ICON-REDWARNING")
        icon:SetTexCoord(0, 18 / 32, 0, 1)
        leaverFrame:Hide()
        self.leaverIcon = leaverFrame
    end
...
function MemberDisplay:SetActivity(activity)
    ...
    if displayData then
        ...
        self.leaverIcon:Hide()
        for i = 1, activity:GetNumMembers() do
          local _, _, _, _, _, isLeaver = LfgService:GetSearchResultMemberInfo(activity:GetID(), i)
          if isLeaver then
            self.leaverIcon:Show()
          end
        end
    else
