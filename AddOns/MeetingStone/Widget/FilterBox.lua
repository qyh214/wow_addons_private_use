...
function FilterBox:SetEnabled(enabled)
    -- 禁用 CheckButton
    self.Check:SetEnabled(enabled)

    -- 禁用 MinBox 和 MaxBox（避免直接调用 SetEnabled）
    self.MinBox:EnableKeyboard(enabled)
    self.MinBox:EnableMouse(enabled)
    self.MaxBox:EnableKeyboard(enabled)
    self.MaxBox:EnableMouse(enabled)

    -- 调整文本颜色
    local textColor = enabled and {1, 1, 1} or {0.5, 0.5, 0.5}
    self.Check:GetFontString():SetTextColor(unpack(textColor))
