...
local LE_LFG_LIST_DISPLAY_TYPE_CLASS_ENUMERATE = Enum.LfgListDisplayType.ClassEnumerate
...
function MainPanel:OpenActivityTooltip(activity, tooltip)
    ...
    if activity:GetLeader() then
        ...
        if score <= 800 then
            tooltip:AddLine(format(L['队长大秘评分：|cffffffff%s|r'], score))
        elseif score <= 1200 then
            tooltip:AddLine(format(L['队长大秘评分：|cff00ff00%s|r'], score))
        elseif score <= 1600 then
            tooltip:AddLine(format(L['队长大秘评分：|cff007fff%s|r'], score))
        elseif score <= 2000 then
            tooltip:AddLine(format(L['队长大秘评分：|cffbf00ff%s|r'], score))
        else
            tooltip:AddLine(format(L['队长大秘评分：|cffff7f00%s|r'], score))
        end
        ...
        if leaderScoreInfo then
            local mapScore = leaderScoreInfo.mapScore or 0;
            if mapScore <= 100 then
                tooltip:AddLine(format("队长当前副本：|cffffffff%d/%d层|r ", mapScore, leaderScoreInfo.bestRunLevel or 0))
            elseif mapScore <= 150 then
                tooltip:AddLine(format("队长当前副本：|cff00ff00%d/%d层|r ", mapScore, leaderScoreInfo.bestRunLevel or 0))
            elseif mapScore <= 200 then
                tooltip:AddLine(format("队长当前副本：|cff007fff%d/%d层|r ", mapScore, leaderScoreInfo.bestRunLevel or 0))
            elseif mapScore <= 2500 then
                tooltip:AddLine(format("队长当前副本：|cffbf00ff%d/%d层|r ", mapScore, leaderScoreInfo.bestRunLevel or 0))
            else
                tooltip:AddLine(format("队长当前副本：|cffff7f00%d/%d层|r ", mapScore, leaderScoreInfo.bestRunLevel or 0))
            end
...
function MainPanel:OpenApplicantTooltip(applicant)
    ...
    elseif score <= 1200 then
        GameTooltip:AddLine(format(L['大秘评分：|cff00ff00%s|r'], score), 1, 1, 1)
    elseif score <= 1600 then
        GameTooltip:AddLine(format(L['大秘评分：|cff007fff%s|r'], score), 1, 1, 1)
    elseif score <= 2000 then
        GameTooltip:AddLine(format(L['大秘评分：|cffbf00ff%s|r'], score), 1, 1, 1)
    else
        GameTooltip:AddLine(format(L['大秘评分：|cffff7f00%s|r'], score), 1, 1, 1)
    end
    -- GameTooltip:AddLine(format(L['大秘评分：|cffffffff%s|r'], score), 1, 1, 1)
    local info = applicant:GetBestDungeonScore()
    if info and info.mapScore and info.mapScore > 0 then
        local mapScore = info.mapScore;
        if mapScore <= 100 then
            GameTooltip:AddLine(format("当前副本: |cffffffff%d/%d层|r ", mapScore, info.bestRunLevel or 0), 1, 1, 1)
        elseif mapScore <= 150 then
            GameTooltip:AddLine(format("当前副本: |cff00ff00%d/%d层|r ", mapScore, info.bestRunLevel or 0), 1, 1, 1)
        elseif mapScore <= 200 then
            GameTooltip:AddLine(format("当前副本: |cff007fff%d/%d层|r ", mapScore, info.bestRunLevel or 0), 1, 1, 1)
        elseif mapScore <= 2500 then
            GameTooltip:AddLine(format("当前副本: |cffbf00ff%d/%d层|r ", mapScore, info.bestRunLevel or 0), 1, 1, 1)
        else
            GameTooltip:AddLine(format("当前副本: |cffff7f00%d/%d层|r ", mapScore, info.bestRunLevel or 0), 1, 1, 1)
        end
