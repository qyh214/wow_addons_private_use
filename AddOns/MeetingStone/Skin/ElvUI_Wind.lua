...
local function defaultcvar()
  ...
  if not S.ESProxy then
    function S:ESProxy(funcName, ...)
        local func = E.Skins[funcName] or S[funcName]
        if func then
            return func(S, ...)
        end
    end
  ...
  function S:MeetingStone()
    ...
    if MainPanel then
      ...
      local close = CreateFrame("Button", "WTMeetingStoneCloseButton", MainPanel, "UIPanelCloseButton, BackdropTemplate")
      ...
      function(self)
        for i = 1, #self.buttons do
          ...
          if button:IsShown() and not button.__windSkin then
            button:StripTextures()
            if button.Icon then
              -- prevent cause error in ElvUI Skin functions
    ...
    if BrowsePanel then
      ...
      -- Meeting Stone EX element (大秘境搜索按钮)
      if BrowsePanel.ResetFilterButton then
        S:ESProxy("HandleButton", BrowsePanel.ResetFilterButton, nil, nil, nil, true, "Transparent")
        BrowsePanel.ResetFilterButton.backdrop:ClearAllPoints()
        BrowsePanel.ResetFilterButton.backdrop:SetOutside(BrowsePanel.ResetFilterButton, -1, -1)
      end
      ...
      if BrowsePanel.NoResultBlocker then
        ...
        if BrowsePanel.NoResultBlocker and BrowsePanel.NoResultBlocker.Label then
            if F and F.SetFontOutline then
                F.SetFontOutline(BrowsePanel.NoResultBlocker.Label)
            elseif E then -- 如果 WindTools 不可用，改用 ElvUI 的设置
                BrowsePanel.NoResultBlocker.Label:SetFont(E.media.normFont, 12, "OUTLINE")
            end
        end
      end
    ...
    local AssociationPanel = NEG.AssociationPanel or MTSAddon and MTSAddon:GetModule("AssociationPanel", true)
    if AssociationPanel then
      -- 皮肤化下拉框
      if AssociationPanel.ActivityDropdown then
        SkinDropDown(AssociationPanel.ActivityDropdown)
      end

      -- 皮肤化按钮
      if AssociationPanel.RefreshButton then
        S:ESProxy("HandleButton", AssociationPanel.RefreshButton, nil, nil, nil, true, "Transparent")
        AssociationPanel.RefreshButton.backdrop:ClearAllPoints()
        AssociationPanel.RefreshButton.backdrop:SetOutside(AssociationPanel.RefreshButton, -1, -1)
      end

      -- 皮肤化搜索框
      if AssociationPanel.SearchBox then
        AssociationPanel.SearchBox:StripTextures()
        S:ESProxy("HandleEditBox", AssociationPanel.SearchBox)
        AssociationPanel.SearchBox.backdrop:ClearAllPoints()
        AssociationPanel.SearchBox.backdrop:SetOutside(AssociationPanel.SearchBox, -1, -2)
      end

      -- 皮肤化列表
      if AssociationPanel.IgnoreList then
        SkinListTitle(AssociationPanel.IgnoreList)
      end

      -- 皮肤化底部按钮
      if AssociationPanel.RecruitIgnore then
        S:ESProxy("HandleButton", AssociationPanel.RecruitIgnore, nil, nil, nil, true, "Transparent")
        AssociationPanel.RecruitIgnore.backdrop:ClearAllPoints()
        AssociationPanel.RecruitIgnore.backdrop:SetOutside(AssociationPanel.RecruitIgnore, -1, -1)
      end

      if AssociationPanel.AddAssociationIgnore then
        S:ESProxy("HandleButton", AssociationPanel.AddAssociationIgnore, nil, nil, nil, true, "Transparent")
        AssociationPanel.AddAssociationIgnore.backdrop:ClearAllPoints()
        AssociationPanel.AddAssociationIgnore.backdrop:SetOutside(AssociationPanel.AddAssociationIgnore, -1, -1)
      end

      if AssociationPanel.AddAssociationProposer then
        S:ESProxy("HandleButton", AssociationPanel.AddAssociationProposer, nil, nil, nil, true, "Transparent")
        AssociationPanel.AddAssociationProposer.backdrop:ClearAllPoints()
        AssociationPanel.AddAssociationProposer.backdrop:SetOutside(AssociationPanel.AddAssociationProposer, -1, -1)
      end

      if AssociationPanel.firstPageButton then
        S:ESProxy("HandleButton", AssociationPanel.firstPageButton, nil, nil, nil, true, "Transparent")
        AssociationPanel.firstPageButton.backdrop:ClearAllPoints()
        AssociationPanel.firstPageButton.backdrop:SetOutside(AssociationPanel.firstPageButton, -1, -1)
      end

      if AssociationPanel.nextPageButton then
        S:ESProxy("HandleButton", AssociationPanel.nextPageButton, nil, nil, nil, true, "Transparent")
        AssociationPanel.nextPageButton.backdrop:ClearAllPoints()
        AssociationPanel.nextPageButton.backdrop:SetOutside(AssociationPanel.nextPageButton, -1, -1)
      end
