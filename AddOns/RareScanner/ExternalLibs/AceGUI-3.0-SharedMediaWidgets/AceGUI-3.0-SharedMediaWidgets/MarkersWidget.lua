...
do
	...
	local function GetContentLine()
		...
		if next(contentFrameCache) then
			...
			local markerButton = CreateFrame("Button", nil, frame)
				markerButton:SetWidth(16)
				markerButton:SetHeight(16)
				markerButton:SetPoint("RIGHT",frame,"RIGHT",-1,0)
				markerButton.frame = frame
			frame.markerButton = markerButton

			local marker = markerButton:CreateTexture(nil, "BACKGROUND")
				marker:SetAllPoints(markerButton)
			frame.marker = marker

			local text = frame:CreateFontString(nil,"OVERLAY","GameFontWhite")
				text:SetPoint("LEFT", check, "RIGHT", 1, 0)
				text:SetPoint("RIGHT", markerButton, "LEFT", -2, 0)
	...
	local function SetDisabled(self, disabled) -- Disable the widget.
		...
		if disabled then
			self.frame:Disable()
			self.marker:SetDesaturated(true)
		else
			self.frame:Enable()
			self.marker:SetDesaturated(false)
		end
	end
	...
	local function ToggleDrop(this)
		...
		if self.dropdown then
			...
			for i, k in ipairs(sortedlist) do
				...
				f.text:SetText(k)
				f.marker:SetTexture(self.list[k])
	...
	local function Constructor()
		...
		local markerButton = CreateFrame("Button", nil, frame)
			markerButton:SetWidth(16)
			markerButton:SetHeight(16)
			markerButton:SetPoint("LEFT",frame.DLeft,"LEFT",26,1)
			markerButton:SetScript("OnClick", WidgetPlaySound)
			markerButton.obj = self
		self.markerButton = markerButton
		frame.text:SetPoint("LEFT",markerButton,"RIGHT",2,0)
