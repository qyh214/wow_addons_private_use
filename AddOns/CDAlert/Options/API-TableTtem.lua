local addonName,ns = ...
local L = ns.L
ns.ItemyOffset = -5	--用于存光环表内行数
ns.ItemrowFrames = {}--用于存表的行框架,方便查询和删除后重新排序

--光环表格行
function ns.AddItemTableButtons(frame,id,name)
	local rowFrame = CreateFrame("Frame", nil, frame)
    rowFrame:SetSize(610, 26)
    rowFrame:SetPoint("TOPLEFT", 5, ns.ItemyOffset)
	ns.ItemrowFrames[id] = rowFrame -- 将行 Frame 存储起来
	...
	local start = ns.AddTableNotes(rowFrame,100,0,100,"ItemIds",id,"start") --创建备注
	...
	local icon = ns.GetItemIcon(id)
	local nametext = ns.AddTableText(rowFrame,rowFrame,222,162)
	nametext:SetText("|T"..icon ..":18|t "..name)
	
	
	local font3 = ns.AddTableNotes(rowFrame,390,0,100,"ItemIds",id,"end") --创建备注
	...
	closeButton:SetScript("OnClick", function()
		CDAlertDB["ItemIds"][id] = nil -- 移除对应的表格内容
        rowFrame:Hide() -- 隐藏这一行
        local newOffset = -5
		for key, row in pairs(ns.ItemrowFrames) do
			...
			ns.ItemyOffset = newOffset
		end
		ns.ItemrowFrames[id] = nil
    end
	end)
