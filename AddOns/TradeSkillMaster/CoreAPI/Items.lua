...
function private.LoadItemCache()
	...
	if hash ~= TSMAPI.Util:CalculateHash(str) then
		TSM:LOG_ERROR("Invalid hash (%s, %s)", tostring(hash), tostring(TSMAPI.Util:CalculateHash(str)))
		return
	end
	...
	if binDataLength % 13 ~= 0 or binDataLength > #str then
		TSM:LOG_ERROR("Invalid bin data length (%s, %s)", tostring(binDataLength), tostring(#str))
		return
	end
	...
	if #names ~= binDataEntries then
		TSM:LOG_ERROR("Invalid names (%s, %s)", tostring(#names), tostring(binDataEntries))
		return
	end
	...
	for i = 0, binDataEntries - 1 do
		...
		if result[itemString] then
			TSM:LOG_ERROR("Duplicate entry (%s)", itemString)
			return
		end
		result[itemString] = {
			name = names[i + 1],
