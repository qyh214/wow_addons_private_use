...
--
--NOTE: use of <> in "SSLink<%s>" caused a conflict with "/ss macro <substitution>" event keys
--      when we gsub below, we would truncate at the first ">" in the <substitution>
--		instead of the final bracket in "SSLink<%s>".
--			"SpeakinSpell:OnClickEditEvent("MACROMACRO_POLY_<RANK":1: unfinished string near '<eof>'
--		To simplify this problem, I changed our private clickhere.lua syntax to "SSLink:{:%s:}:"
--		though possible, this is not likely to be used in anyone's custom macro names
--		and this prevents the need for a patch function to remove <brackets> from pre-existing event key names
--
function SpeakinSpell_SetItemRef(link, text, button)
	local funcname = "SpeakinSpell_SetItemRef"	
	SpeakinSpell:DebugMsg(funcname, "link:"..tostring(link)) -- local link = string.format( "SSLink:{:%s:}:", LuaCode )
...
function SpeakinSpell:MakeClickHereLink(ClickHere, LuaCode, Color)
	...
	if Color == nil then
		if not SpeakinSpellSavedData.Colors then
			SpeakinSpellSavedData.Colors = self:CopyTable(SpeakinSpell.DEFAULTS.SpeakinSpellSavedData.Colors)
		end
		if SpeakinSpellSavedData.Colors.ClickHere == nil then
			SpeakinSpellSavedData.Colors.ClickHere = SpeakinSpell.DEFAULTS.SpeakinSpellSavedData.Colors.ClickHere
		end
