...
function SpeakinSpell:Network_Init()
	if not SpeakinSpellSavedDataForAll.Networking then
		-- Ensure the networking options data table exists
		-- I hate to do this here, but an error was reported in 4.0.6.01
		-- SpeakinSpellSavedDataForAll.Networking was nil at this point during OnVariablesLoaded
		-- I'm not doing this during every load cycle anymore as of 4.0.3.01 because it seemed unnecessary
		-- and was slowing down game login times when the saved data table was large
		SpeakinSpellSavedDataForAll.Networking = {}
...
function SpeakinSpell:Network_GetCompressedNewEventsDetected( AlreadyKnownToFarEnd ) --note AlreadyKnownToFarEnd can be a nil table
	...
	--TODOFUTURE: this is an expensive big module to load for this, is it really necessary? seems like it is
