...
function SpeakinSpell:OnSlashCommand(input)
	...
	self:ShowHelpTopic( HELPFILE.SLASH_COMMANDS )
...
-- returns ete, ismacro
function SpeakinSpell:GetETEForSSMacro(input, de)
	...
	--		so it won't be treated as a macro, and will be preserved as-is by the expansion process
	local LineBreaks = { "\n", "<newline>", L["<newline>"] }
	for _,newline in pairs(LineBreaks) do
		local index = string.find(macro, newline)
		if (index and index>=0) then
			return nil, false
		end
	end

	-- support "/ss macro <targetclass>" -> "/ss macro Paladin" etc.
	if de then
		macro = self:FormatSubs(macro, de)
	end

	local key = self:Keyify( "MACRO"..tostring(macro) )
	--self:DebugMsg(funcname, "key:"..key)
	return self:GetActiveEventTable()[key], true --can return (nil, true) for undefined macros
end
...
function SpeakinSpell:ProcessRecursiveUserMacro(input, ParentDetectedEvent)
	...
	local ete = self:GetETEForSSMacro(input, ParentDetectedEvent)
	if not ete then
