...
function LibSmartComm:GetBroadcastChannels()
	...
	if zoneType=="pvp" then
		tinsert(BroadcastChannels, "INSTANCE_CHAT")
...
function LibSmartComm:_NewAddon( addonid, AddonObject, clientversion, dataversion )
	...
	-- RegisterAddonMessagePrefix is needed as of WoW patch 4.1
	-- see http://us.battle.net/wow/en/forum/topic/2228413591
	--TODOLATER: IsAddonMessagePrefixRegistered to check for duplicates
	--TODOLATER: check the return value - RegisterAddonMessagePrefix can fail if more than 512 have been registered
	--TODOLATER: is there any use for GetRegisteredAddonMessagePrefixes?
	C_ChatInfo.RegisterAddonMessagePrefix(addonid)
...
-- distribution
--		a channel valid for SendAddonMessage (GUILD, RAID, PARTY, INSTANCE_CHAT, or WHISPER)
-- target
