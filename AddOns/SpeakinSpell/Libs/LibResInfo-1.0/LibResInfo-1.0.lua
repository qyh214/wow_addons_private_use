...
function eventFrame:UNIT_AURA(event, unit)
	...
	if not isDead[guid] then
		...
		local reincarnation = AuraUtil.FindAuraByName(REINCARNATION, unit, "HARMFUL")
		if reincarnation ~= hasReincarnation[guid] then
			...
			local resurrecting, _, _, _, _, expires = AuraUtil.FindAuraByName(RESURRECTING, unit, "HARMFUL")
