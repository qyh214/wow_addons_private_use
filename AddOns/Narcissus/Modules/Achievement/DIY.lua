local DIYContainer, EditorContainer, DIYCards, NewEntryButton, FloatingCards;
local SelectedCard;
...
local function CreateCustomFloatingCard(self)
    ...
    local uiScale = DIYContainer:GetEffectiveScale();
    ...
    local positionFrame = FloatingCards.positionFrame;
...
function FormatCardByIndex(buttonIndex, name, description, icon, points, date, rewardText, isAccountWide)
    ...
    if not button then
        button = CreateFrame("Button", nil, DIYContainer.ScrollChild, "NarciAchievementCardLargeTemplate");
...
initialize:SetScript("OnEvent", function(self, event, ...)
    if event == "PLAYER_ENTERING_WORLD" then
        ...
        FloatingCards = NarciFloatingCardsContainer;
        ...
        ReskinButton = NarciAchievement_ReskinButton;
