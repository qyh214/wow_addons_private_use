...
function NarciScreenshotInfoMixin:OnMouseWheel(delta)
    Narci_WorldMapFrame:OnMouseWheel(delta)
end

function NarciScreenshotInfoMixin:UpdateText(object, str)
    UIFrameFadeOut(object, 0.15, object:GetAlpha(), 0);
    UIFrameFadeOut(self.MinimapNameHighlight, 0.15, self.MinimapNameHighlight:GetAlpha(), 0);
    if str then
        After(0.15, function()
            object:SetText(str);
            UIFrameFadeIn(object, 0.15, 0, 1);
            UIFrameFadeIn(self.MinimapNameHighlight, 0.15, 0, 1);
        end)
    end
end
...
function NarciScreenshotInfoMixin:GetPlayerPosition()
    ...
    if mapID then
        ...
        if zoneName ~= self.zoneName then
            ...
            self:UpdateText(self.MinimapName, zoneName);
        end
        if mapID ~= self.mapID and mapInfo then
            local mapName = mapInfo.name;
            self.mapID = mapID;
            self.mapName = mapName;
            self.MapName:SetText(upper(mapName));
...
local ZOOM_LEVEL_MAX = 1.5^2;
local ZOOM_LEVEL_MIN = 1/1.5^2;
local DataProvider
NarciWorldMapMixin = {};
...
function NarciWorldMapMixin:OnLoad()
    ...
    local Pin = self.Pin;
    Pin:SetTexture("Interface/AddOns/Narcissus/Art/Widgets/WorldMap/Pin", nil, nil, "TRILINEAR");
    Pin:SetDrawLayer("OVERLAY", 3);
    self.numTileAlongX = 4;
    self.numTileAlongY = 3;
    self:SetSize(256 * 4, 256 * 3);
    self.zoomLevelMax = ZOOM_LEVEL_MAX;
    self.zoomLevelMin = ZOOM_LEVEL_MIN;
    self.zoomLevel = 1;
...
function NarciWorldMapMixin:UpdateOffsetRange()
    self.maxOffsetX = self.layerWidth - 128 /self.zoomLevel;
    self.maxOffsetY = self.layerHeight - 128 /self.zoomLevel;
    self:UpdateMapAndPinOffset();
end

function NarciWorldMapMixin:SetZoomLevel(zoomLevel)
    self:SetScale(zoomLevel);
    local normalizedSize;
    if zoomLevel < 1 then
        normalizedSize = 36* 1/sqrt(zoomLevel);
    else
        normalizedSize = 36* 1/zoomLevel;
    end
    self.Pin:SetSize(normalizedSize, normalizedSize);
...
function NarciWorldMapMixin:UpdateScale(newZoomLevel)
    print(newZoomLevel)
    if newZoomLevel > self.zoomLevelMax then
        newZoomLevel = self.zoomLevelMax;
    elseif newZoomLevel < self.zoomLevelMin then
        newZoomLevel = self.zoomLevelMin;
    end
...
function NarciWorldMapMixin:OnMouseWheel(delta)
    local newZoomLevel;
    if delta > 0 then
        newZoomLevel = min(self.finalZoomLevel*1.5, self.zoomLevelMax);
    else
        newZoomLevel = max(self.finalZoomLevel/1.5, self.zoomLevelMin);
    end

    SmoothScaleFast(self, self.zoomLevel, newZoomLevel);
end

function NarciWorldMapMixin:AcquireBaseTextures(childFrame)
    local a, b = self.numTileAlongX, self.numTileAlongY;
    local tex;
    if not childFrame.texs then
        childFrame.texs = {}
    end
    ...
    for i = 1, (a * b) do
        tex = texs[i];
        if not tex then
            tex = childFrame:CreateTexture(nil, "BACKGROUND");
            tex:SetSize(self.tileSizeX, self.tileSizeY);
            tinsert(texs, tex);
        end
        if i == 1 then
            tex:SetPoint("TOPLEFT", childFrame, "TOPLEFT", 0, 0);
        elseif i % a == 1 then
...
function NarciWorldMapMixin:AcquireTexureByIndex(childFrame, index)
    if not childFrame.texs then
        childFrame.texs = {};
    end

    if childFrame.texs[index] then
        return childFrame.texs[index]
    else
        local tex = childFrame:CreateTexture(nil, "BACKGROUND");
        childFrame.texs[index] = tex;
        return tex
    end
end
...
function NarciWorldMapMixin:CreateTrailPool(numTrails)
    ...
    for i = 1, numTrails do
        ...
        if not trail then
            ...
            trail:SetSize(18, 18);
            trail:SetTexture("Interface/AddOns/Narcissus/Art/Widgets/WorldMap/Trail", nil, nil, "TRILINEAR");
        end
    end
...
function NarciWorldMapMixin:SetOffset(x, y)
    local minOffset = 128/self.zoomLevel;
    ...
    self:ClearAllPoints();
    self:SetPoint("TOPLEFT", self:GetParent(), "TOPLEFT", minOffset - x, y - minOffset);     --Tile Size / 2
end
...
function NarciWorldMapMixin:UpdateMapAndPinOffset()
    ...
    self.Pin:SetPoint("CENTER", self.OverlayFrame, "TOPLEFT", self.offsetX, - self.offsetY);
    self.Pin:SetRotation(GetPlayerFacing() or 0);
end
...
function NarciWorldMapMixin:SetNewTrail()
    ...
    After(0.5, function()
        ...
        trail:SetPoint("CENTER", self.OverlayFrame, "TOPLEFT", x, y);
...
function NarciWorldMapMixin:SetCurrentMap(forceUpdate)
    ...
    if mapID ~= self.mapID or forceUpdate then
        ...
        local baseTextures = self:AcquireBaseTextures(self.BaseFrame);
        ...
        if exploredMapTextures then
            ...
            for _, artInfo in pairs(exploredMapTextures) do
                ...
                if textureWidth and textureWidth ~= 0 then
                    ...
                    for i = 1, numTex do
                        tex = self:AcquireTexureByIndex(OverlayFrame, texIndex);
                        tex:ClearAllPoints();
    ...
    local pin = self.Pin;
    pin:ClearAllPoints();
    pin:SetPoint("CENTER", self.OverlayFrame, "TOPLEFT", offsetX, -offsetY);
...
TestFrame:AddUnit("player", "Interface/AddOns/Narcissus/Art/Widgets/WorldMap/Pin", 16, 16, 1, 1, 1, 1, 10, true)
--]]
